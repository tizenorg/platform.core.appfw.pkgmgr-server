...
static void __process_enable_app(pm_dbus_msg *item)
{
	...
	__send_app_signal(item->req_id, item->pkg_type, item->pkgid, item->appid, "start", "enable_app");
...
static void __process_disable_app(pm_dbus_msg *item)
{
	...
	__send_app_signal(item->req_id, item->pkg_type, item->pkgid, item->appid, "start", "disable_app");
...
static void __process_enable_global_app(pm_dbus_msg *item)
{
	...
	__send_app_signal(item->req_id, item->pkg_type, item->pkgid, item->appid, "start", "enable_global_app");
...
static void __process_disable_global_app(pm_dbus_msg *item)
{
	...
	__change_item_info(item, GLOBAL_USER);
	__send_app_signal(item->req_id, item->pkg_type, item->pkgid, item->appid, "start", "disable_global_app");
