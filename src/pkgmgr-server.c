...
static void __process_enable_global_app(pm_dbus_msg *item)
{
	...
	ret = pkgmgr_parser_update_global_app_disable_info_in_db(item->pkgid, item->uid, 0);
...
static void __process_disable_global_app(pm_dbus_msg *item)
{
	...
	ret = pkgmgr_parser_update_global_app_disable_info_in_db(item->pkgid, item->uid, 1);
