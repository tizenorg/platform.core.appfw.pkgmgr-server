...
static void __send_app_signal(uid_t uid, const char *req_id, const char *pkg_type, const char *pkgid,
...
static void __process_enable_global_app_for_uid(pm_dbus_msg *item)
{
	int ret = -1;
...
static void __process_disable_global_app_for_uid(pm_dbus_msg *item)
{
	int ret = -1;
...
gboolean queue_job(void *data)
{
	...
	case 0:	/* child */
		...
		case PKGMGR_REQUEST_TYPE_ENABLE_GLOBAL_APP_FOR_UID:
			__process_enable_global_app_for_uid(item);
			break;
		case PKGMGR_REQUEST_TYPE_DISABLE_GLOBAL_APP_FOR_UID:
			__process_disable_global_app_for_uid(item);
			break;
