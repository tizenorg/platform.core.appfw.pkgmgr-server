...
static int __process_set_restriction_mode(pm_dbus_msg *item)
{
	...
	ret = __set_restriction_mode(item->uid);
...
static int __process_unset_restriction_mode(pm_dbus_msg *item)
{
	...
	ret = __unset_restriction_mode(item->uid);
...
static int __process_check_restriction_mode(pm_dbus_msg *item)
{
	int ret;
	int result = -1;

	ret = __check_restriction_mode(item->uid, &result);
...
gboolean queue_job(void *data)
{
	...
	case PKGMGR_REQUEST_TYPE_UNSET_RESTRICTION_MODE:
		...
		break;
	case PKGMGR_REQUEST_TYPE_CHECK_RESTRICTION_MODE:
		ret = __process_check_restriction_mode(item);
		break;
	default:
		ret = -1;
		break;
