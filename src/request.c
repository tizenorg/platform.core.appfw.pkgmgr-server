...
static int __handle_request_update_app_splash_screen(uid_t uid,
		GDBusMethodInvocation *invocation, GVariant *parameters,
		int flag)
{
	...
	char *reqkey = NULL;
	int ret = PKGMGR_R_OK;
	int req_type = PKGMGR_REQUEST_TYPE_ENABLE_APP_SPLASH_SCREEN;
	...
	if (!flag)
		req_type = PKGMGR_REQUEST_TYPE_DISABLE_APP_SPLASH_SCREEN;
		...
		gpointer user_data)
{
	...
	if (g_strcmp0(method_name, "install") == 0)
		...
		ret = __handle_request_update_app_splash_screen(uid,
