		...
		int flag)
{
	...
	req_type = flag ? PKGMGR_REQUEST_TYPE_ENABLE_APP_SPLASH_SCREEN :
...
static int __handle_request_enable_app_splash_screen(uid_t uid,
		GDBusMethodInvocation *invocation, GVariant *parameters)
{
	return __update_app_splash_screen(uid, invocation, parameters, 1);
}

static int __handle_request_disable_app_splash_screen(uid_t uid,
		GDBusMethodInvocation *invocation, GVariant *parameters)
{
	return __update_app_splash_screen(uid, invocation, parameters, 0);
}
