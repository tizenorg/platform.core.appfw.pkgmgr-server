...
static const char instropection_xml[] =
	...
	"      <arg type='s' name='pkgtype' direction='in'/>"
	"      <arg type='s' name='pkgid' direction='in'/>"
	"		 <arg type='s' name='movetype' direction='in'/>"
	"      <arg type='i' name='ret' direction='out'/>"
	"    </method>"
		...
		GDBusMethodInvocation *invocation, GVariant *parameters)
{
	...
	g_variant_get(parameters, "(u&s&s&s)", &target_uid, &pkgtype, &pkgid, &move_type);
	if (target_uid == (uid_t)-1 || pkgtype == NULL || pkgid == NULL || move_type == NULL) {
		g_dbus_method_invocation_return_value(invocation,
